/*
ou are given a string text. We want to display text on a screen of width w and height h. You can choose any font size from array fonts, which contains the available font sizes in ascending order.
...
class Solution {
    ...
    //BS
    // 执行用时：3 ms, 在所有 Java 提交中击败了59.46%的用户
    // 内存消耗：50.9 MB, 在所有 Java 提交中击败了37.84%的用户
    // 通过测试用例：19 / 19
    public int maxFont(String text, int w, int h, int[] fonts, FontInfo fontInfo) {
        ...
        while(l <= r){
            int m = l + (r - l) / 2;
            if (check(fontInfo, fonts[m],text,w,h)){
                ans = fonts[m];
                l = m + 1;
            }else{
                r = m - 1;
    ...
    public boolean check(FontInfo fontInfo,int fontSize,String text, int w, int h){
        int textSize = text.length();
        int textW = 0;
        for(int i = 0;i < text.length();i++){
            textW += fontInfo.getWidth(fontSize, text.charAt(i));
        }         
        int textH = fontInfo.getHeight(fontSize);
        
        if (textW > w || textH > h) return false;
        return true;
